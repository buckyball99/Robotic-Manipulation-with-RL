...
def get_vars(scope):
  ...
  # get all variables within a scope
def get_vars_numpy(scope):
  return np.array([x for x in tf.global_variables() if scope in x.name])
...
def CreateNetworks(
    s, a,
    num_actions,
    action_max,  # 1. To multiply the output with , scaled_q
    hidden_sizes=(300,),
...
def ddpg(
  ...
  # target_update = tf.group([tf.assign(v_targ, decay*v_targ + (1 - decay)*v_main) for v_main, v_targ in zip(get_vars('main'), get_vars('target'))])
  target_update = [tf.assign(v_targ, decay*v_targ + (1 - decay)*v_main) for v_main, v_targ in zip(get_vars('main'), get_vars('target'))]
  # target_update = [v_targ.assign(tf.multiply(v_targ,decay) + tf.multiply(v_main,(1-decay)) ) 
	# 				        for v_targ,v_main in zip(tf.trainable_variables('target'), tf.trainable_variables('main'))]
  ...
  def get_action(s, noise_scale):
    ...
    a1 = sess.run(mu, feed_dict={X: s.reshape(1,-1)})
    # print("mu actions - a1:", a1)
    a = a1[0]
  ...
  for i_episode in range(num_train_episodes):
    ...
    for _ in range(episode_length):
      ...
      # print("Batch order :", batch)
      # print("S shape is ",X.shape)
